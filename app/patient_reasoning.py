...
class PatientDecom1:
    ...
    def get_all_source(self):
        ...
        if not self.patient.get('ehr_note'):
            self.patient = patientEnrich(self.patient).enrich_ehr()
        if not self.patient.get('lab_result'):
            self.patient = patientEnrich(self.patient).enrich_lab()
        if not self.patient.get('nerve_center_data'):
            self.patient = patientEnrich(self.patient).enrich_Nervecentre()
        if not self.patient.get('medilogik_data'):
            self.patient = patientEnrich(self.patient).enrich_Medilogik()
        if not self.patient.get('viper_data'):
            self.patient = patientEnrich(self.patient).enrich_Viper()
        if not self.patient.get('ice_data'):
            self.patient = patientEnrich(self.patient).enrich_ICE()
        if not self.patient.get('bighand_data'):
            self.patient = patientEnrich(self.patient).enrich_BigHand()
        if not self.patient.get('vue_data'):
            self.patient = patientEnrich(self.patient).enrich_VueExplore()

    async def _data_analysis(self, query):
        data_s = [
            "ehr_note",
            "lab_result",
            "nerve_center_data",
        ...
        data_source_str = ""
        for d in data_s:
            if self.patient.get(d):
                data_source_str += f"# Source: {d}\n"
                data_source_str += f"## {d}\n{json.dumps(self.patient.get(d), indent=4)}\n\n"

        prompt = f"User query : {query.get('user_query')}\n\ndata source :\n{data_source_str}\n\nAnalyze the patient data and perform the user query."
        res_obj = CRunner(
            agent = agent_list.data_analyst_agent,
            prompt = prompt,
        )
        ...
        if not self.patient.get('data_analyst'):
            self.patient['data_analyst'] = [
                {
            ...
            self.patient['data_analyst'].append(
                {
                    "query_id" : query.get('query_id'),
                    "query" : query,
                    "result" : res
